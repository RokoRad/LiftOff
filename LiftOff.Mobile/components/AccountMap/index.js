  ...
  return (
    <View style={styles.wrapper}>
      ...
      <MapView zoomEnabled={true} style={{ flex: 1 }} provider={PROVIDER_GOOGLE} customMapStyle={style} cacheEnabled={true}
        ...
        {markers.map((marker) => <MapView.Marker coordinate={marker.flightLocation} key={index} image={require('../../images/map/pin.png')}/> )}
      </MapView>
    </View>
  )
}
...
//{markers.map((marker, index) => <MapView.Marker coordinate={marker[index].flightLocation} key={index} image={require('../../images/map/pin.png')}/> )}