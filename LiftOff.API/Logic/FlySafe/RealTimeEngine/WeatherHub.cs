...
namespace LiftOff.API.Logic.FlySafe.RealTimeEngine
{
	...
	public class WeatherHub : Hub
	{
		...
		public void InitiateConnection(TimeLocation timeLocation, string droneName, string units)
		{
            //debug
            //timeLocation.Time = System.DateTime.Now;

            if (timeLocation.LocationIsValid() && timeLocation.TimeIsValid())
                _realTimeConnections.Add(new WeatherGetter(timeLocation, droneName, units, Context.ConnectionId));
		}
        ...
        public void UpdateDrone(string droneName)
        {
            var clientWeatherGetter = _realTimeConnections.First(wg => wg.GetClient().ConnectionId == Context.ConnectionId);

            clientWeatherGetter.UpdateClientDrone(droneName);
        }
